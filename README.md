...
The single Template file, "contactInfoData.json", that accesses the secret Data file is hard-coded to access the YAML object "contactInfoSecretData" within "_data/secretData.yml" and this cannot be changed by either front matter or config variables. </br>
...
CreateResumes.sh **MUST** be run in project root to function correctly. Files are output from CreateResumes.sh with file names in the following format: "10-31-24_14-02-24_TestResume" into the resume's "_outputResumes" directory; both file names and output location are hard-coded. (Filename generation is hard-coded so that any number of resumes can be generated without any having matching filenames and so that the newest resume will be the last item in the directory).
